if (Meteor.isClient) {
  ...
  Template.mstPicker.created = function() {
    this.filter = new ReactiveTable.Filter('mst', ['Time']);
  }

  Template.mstPicker.helpers({
      mstOnly: function() { return Session.get('mstSet'); }
  })

  Template.mstPicker.events({
    'change .mstOnly input': function (event, template) {
      Session.set('mstSet', event.target.checked);
      if (event.target.checked) {
        template.filter.set({'$lte': new Date("September 05, 2015 12:00:00")});
      } else {
        template.filter.set('');
      }
    }
