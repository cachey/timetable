if (Meteor.isClient) {
  ...
  Template.futureOnlyPicker.created = function() {
    this.filter = new ReactiveTable.Filter('future', ['Date']);
  };
  ...
  Template.mstPicker.created = function() {
    this.filter = new ReactiveTable.Filter('mst', ['Date']);
  };

  Template.mstPicker.events({
    'click .btn-default': function (event, template) {
      if (event.target.innerText === 'MST') template.filter.set({'$lte': new Date(Session.get('halfway'))});
      else if (event.target.innerText === 'EOS') template.filter.set({'$gte': new Date(Session.get('halfway'))});
  ...
  Template.nameFilter.created = function () {
    this.filter = new ReactiveTable.Filter('search', ['Title']);
  };
